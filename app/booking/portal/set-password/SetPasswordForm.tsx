...
export function SetPasswordForm({ token }: { token: string | null }) {
  ...
  useEffect(() => {
    if (!token) setError("This link is missing its token. Request a new password reset email.");
  }, [token]);
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/api/booking/portal/set-password${qs}`, {
        method: "POST",
      ...
      window.location.href = `/booking/portal${qs}`;
    } catch (err) {
  ...
  return (
    <form onSubmit={submit} className="space-y-4 bg-white/[0.02] border border-white/10 rounded-lg p-6">
      <label className="block">
        <span className="block text-xs text-gray-500 mb-1">New password (min 8 characters)</span>
        <input
          type="password"
          required
          minLength={8}
